...
# note:
# huggingface TF-codeT5 implementation has issues when mixed precision is enabled
...
def download_dataset(cache_dir):
    ...
    _url = "https://dataset.jsonl" # download dataset
    dataset_path = tf.keras.utils.get_file("dataset.jsonl", origin=_url, cache_dir=cache_dir, cache_subdir=cache_dir)
    return dataset_path
...
def predict_from_dataset(args):
    ...
    dataset = load_dataset('json', data_files='https://dataset.jsonl')
    # train test split
...
predict_from_text(args, "Write a function to find the largest integers from a given list of numbers using heap queue algorithm."); print()
predict_from_text(args, "Write a function to get the n smallest items from a dataset."); print()
